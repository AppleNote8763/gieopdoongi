...
${input.completedTasks ? `\n[기존 완료된 학습/과제 항목 (보존 필수)]\n${input.completedTasks.map(t => `- ${t.title}`).join('\n')}\n* 주의: 사용자가 이미 완료한 위 항목들은 체크리스트(checklist) 배열의 앞부분에 반드시 포함시키고 (done: true로 설정), 남은 기간 동안 수행해야 할 새로운 항목들만 추가로 생성해줘.` : ""}
...
2. 자격증 및 시험 준비 전략:
   ...
   - 자격증 시험 일정이 가까운(임박한) 경우, 자격증 공부 및 시험 준비 태스크의 우선 순위('priorities')를 가장 높게 설정하고 'checklist'에서도 최상단으로 배치해줘.
